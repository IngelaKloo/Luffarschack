...
Board Board::makeResultOfMove( MoveInfo& move)//om man kan g�ra s� att denna h�ller p� med referenser ist�llet f�r att returnera en hel board vore det bra
{
...
void Board::makeMove(MoveInfo& move, int depth)
{
	...
	if(depth==0 && move.getPlacement()!=MoveInfo::UNVALID)
	{
		...
		if(!tiles[activatedBoardIndex]->activateBoard())//f�rs�ker aktivera det br�de som ska aktiveras om det inte �r taget
		{
			bool successfullActivation=false;
			/*om det �r taget och inte blir aktiverat...*/
			for(int i=0; i<9; ++i)
			{
				tiles[i]->activateBoard();//aktivera alla br�den som g�r
			}
		}
	}
}
...
int Board::determineValueForAI(Tile::Owner sign, int depth)//misst�nkt
{
	Tile::Owner opponent;
	if(sign==Tile::CROSS)
	{
	...
	if(owner==Tile::NONE)
	{
		...
		for(unsigned int i=0; i<allPossibleRows.size(); ++i)
		{
			...
			/*om denna raden kan tas av motst�ndaren*/
	...
	{
		/*om man inte har st�d s� h�r l�ngt ner*/
		assert(false);
	}
	if(owner==Tile::NONE)
	{
	...
	else//ignorera tiles om br�det �r vunnet
	{
		return valueOfBoard;
	}
}
